...
#define TPMS_MESSAGE_SIZE		(uint32_t)(8)			/* TPMS message size */
...
void rfRx_DeviceConfig(void) {
	...
	Nrf905Lld_SetRxPayloadWidth(ENrf905LldPayloadWidth_8);
	/*
	 * TODO: Determine the address and its width
	 */
	Nrf905Lld_SetRxAddressWidth(ENrf905LldAddressWidth_1);
	Nrf905Lld_SetDeviceIdentity(NULL, 0);
	/*
	 * TODO: Assert no output clock
	 */
	Nrf905Lld_EnableOutputClock(false);
...
void rfRx_HandleMessage(void) {
	...
	static uint8_t rxBufTbl[TPMS_MESSAGE_SIZE]; /* SPI Rx buffer */

	/* Wait for notification from EXTI ISR */
	if(0UL < ulTaskNotifyTake(pdTRUE, 0)) {

		/* Read the data */
		Nrf905Lld_ReadRxPayload(rxBufTbl, TPMS_MESSAGE_SIZE);
